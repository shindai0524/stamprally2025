...
const HELP_IMAGES = {
  ...
  qr:   ["image/qr11.png","image/qr22.png"]
};
...
function updateHelpView() {
  ...
  // --- QR 2枚目だけ特別レイアウト ---
  const viewer = document.querySelector(".help-viewer");
  const qrText = document.getElementById("qrSecondText");

  if (helpState.category === "qr" && helpState.index === 1) {
    viewer.classList.add("qr-second");   // CSS専用クラスを付与
    qrText?.classList.remove("hidden");  // 説明文を表示
  } else {
    viewer.classList.remove("qr-second");
